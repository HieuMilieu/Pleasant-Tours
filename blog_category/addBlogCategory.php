...
<?php
    ...
    if (!empty($_POST)) {
            ...
            if (!empty($name)) {
                    ...
                    //Luu vao database
                    if ($id == '') {
                            $sql = 'insert into blog_category(name, created_at, updated_at) values ("'.$name.'", "'.$created_at.'", "'.$updated_at.'")';
                    } else {
                            $sql = 'update blog_category set name = "'.$name.'", updated_at = "'.$updated_at.'" where id = '.$id;
                    }

                    execute($sql);

                    header('Location: ../blog_category/blogCategory.php');
                    die();
            }
    }
...
<body>
    ...
    <div id="right-panel" class="right-panel">
		...
		<div class="panel-heading">
                    <h2 class="text-center">Thêm/Sửa Danh Mục Blog</h2>
		</div>
